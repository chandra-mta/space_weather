...
$SPACE_file = 'https://services.swpc.noaa.gov/text/ace-epam.txt'; 
$RACE_file = './ace_fast.archive';
$ephem_file = '/data/mta4/proj/rac/ops/ephem/gephem.dat';
$fluence_file = './fluace_fast.dat';
$fluence_file_bak = './fluace_fast.dat.good';
...
open (FF, $fluence_file_bak) or die scalar(gmtime)." ACE fluence file $fluence_file_bak not found!\n";
@ff = <FF>;
chomp @ff;
@fh = grep { /^#/ } @ff;
@fn = grep { /^\d/ } @ff;
