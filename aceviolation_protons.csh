...
#set lock = "./prot_violate"
    ...
    if ( -f $lock) then
      ...
      if (! -s $block) then
        #tmp echo "Telecon now on 1-877-521-0441 111165#" >> $lock
        #tmp echo "This message sent to sot_red_alert" >> $lock
        echo "This message sent to sot_ace_alert" >> $lock
        #tmp cat $lock | mailx -s "ACE_p3 telecon now" sot_red_alert
