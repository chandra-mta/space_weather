...
x2=x2(s)
time=time(s)
b=where(jday ge max(jday)-t_arch+1,bnum)
...
ymax=max([de1,de4])
print,ymin,ymax
plot,time,de1,background=back_color,color=grid_color, $
  xstyle=1,ystyle=1,xrange=[xmin,xmax],yrange=[ymin,ymax],/ylog,/nodata, $
  charsize=csize,ymargin=[-0.6,4], xmargin=[xleft,xright], $
  xticks=nticks-1,xtickv=doyticks, $
  xtickformat='no_axis_labels',xminor=12
b=where(estat eq 0)
oplot,time(b),de1(b),psym=3,color=de1_color
oplot,time(b),de4(b),psym=3,color=de4_color
yline=1L
...
xyouts,xmin,ymax*1.5,"ACE RTSW (Estimated) EPAM",/data,charsize=0.9
...
xyouts,xlabp1,0.78,"Electrons/cm2-s-sr-MeV",orient=90,align=0.5,/norm
xyouts,xlabp2,0.74,'38-53 ',orient=90,color=de1_color, $
       /norm,charsize=chancsize
xyouts,xlabp2,0.81,'175-315 keV',orient=90,color=de4_color, $
       /norm,charsize=chancsize

ymin=min(p7(where(p7 ge 0.01)))
ymax=max([p2,p3,p5,p6,p7])
plot,time,p5,background=back_color,color=grid_color, $
  xstyle=1,ystyle=1,xrange=[xmin,xmax],yrange=[ymin,ymax],/ylog,/nodata, $
  charsize=csize,ymargin=[-8,1], xmargin=[xleft,xright], $
  xticks=nticks-1,xtickv=doyticks, $
  xtickformat='no_axis_labels',xminor=12
b=where(pstat eq 0)
oplot,time(b),p2(b),psym=3,color=p2_color
...
while(yline lt ymax) do begin
  ...
  yline=yline*10
endwhile ; while(yline lt ymax) do begin
xyouts,xlabp1,0.42,'Protons/cm2-s-sr-MeV',orient=90,align=0.5,/norm
xyouts,xlabp2,0.17,'47-68 ',orient=90,color=p2_color, $
       /norm,charsize=chancsize
xyouts,xlabp2,0.27,'115-195 ',orient=90,color=p3_color, $
       /norm,charsize=chancsize
xyouts,xlabp2,0.38,'310-580 ',orient=90,color=p5_color, $
       /norm,charsize=chancsize
xyouts,xlabp2,0.48,'761-1220 ',orient=90,color=p6_color, $
       /norm,charsize=chancsize
xyouts,xlabp2,0.58,'1060-1910 keV',orient=90,color=p7_color, $
       /norm,charsize=chancsize

plot,time,aindex,background=back_color,color=grid_color, $
  xstyle=1,ystyle=1,xrange=[xmin,xmax],yrange=[0,2],/nodata, $
  charsize=csize,ymargin=[3,8.4],xtitle='UTC (days)', xmargin=[xleft,xright], $
  xticks=nticks-1,xtickv=doyticks, $
  xtickname=xticklab,xminor=12,yticks=2,yminor=5
b=where(pstat eq 0)
oplot,time(b),aindex(b),psym=3,color=aindex_color
xyouts,xlabp2,0.10,"Anisotropy",orient=90,/norm,align=0.5,charsize=0.8
xyouts,xlabp2+(xlabp2-xlabp1),0.10,"Index",orient=90,/norm, $
...
write_gif,'/data/mta4/www/mta_ace_plot.gif',tvrd()
